...
public class ChipBubbleTextWatcher implements TextWatcher {
	...
	public ChipBubbleTextWatcher(Context context, MultiAutoCompleteTextView multiAutoCompleteTextView) {
		super();
		...
		if (temp.length() > 0) {
			if (temp.charAt(temp.length() - 1) == ',') {
				if (error == 0) {
					...
					GeneralFunctions.setChips(context, chiptxt, multiAutoCompleteTextView);
