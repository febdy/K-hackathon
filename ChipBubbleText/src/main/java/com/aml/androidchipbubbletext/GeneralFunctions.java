...
public class GeneralFunctions {
	...
	public static void setChips(final Context context, CharSequence typedText,
			final MultiAutoCompleteTextView mutiView) {
			...
			for (String c : chips) {
				...
				if (c.length() > 2) {
					...
					BitmapDrawable bmpDrawableDummy = getBitmapDrawable(context, " ", false, 4);
					ssb.setSpan(new ImageSpan(bmpDrawableDummy), x + c.length(), x + c.length() + 1, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);

				}else{
					
					BitmapDrawable bmpDrawableText = getBitmapDrawable(context, c, true, 1);
	...
	private static BitmapDrawable getBitmapDrawable(Context context, String str, boolean close, int count){
		...
		if ((count == 1) && close) {
			...
			drawable.setColor(Color.parseColor("#FF9999"));
		}else if (count == 1) {
			textView.setPadding(10, 1, 0, 5);
			...
			textView.setPadding(0, 1, 0, 5);
			textView.setBackgroundColor(Color.parseColor("#00FF9999")); // space
		} else {
			textView.setPadding(0, 1, 0, 5);
		}
