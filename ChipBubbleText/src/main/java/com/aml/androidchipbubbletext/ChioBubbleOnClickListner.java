...
public class ChioBubbleOnClickListner implements OnClickListener {
	...
	public ChioBubbleOnClickListner(Context context, MultiAutoCompleteTextView autoCompleteTextView) {
		super();
		...
		if ((i>0) && (i < temp.length())) {
			...
			if (flag == 1) {
				...
				if (cs.toString().length()>0) {
					GeneralFunctions.setChips(context, cs, autoCompleteTextView);
				}				
			}			
		}else{
			autoCompleteTextView.setSelection(autoCompleteTextView.length());
